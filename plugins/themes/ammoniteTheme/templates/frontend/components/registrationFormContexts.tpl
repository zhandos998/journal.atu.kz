{**
 * @file plugins/themes/ammoniteTheme/templates/frontend/components/registrationFormContexts.tpl
 *
	...
	<fieldset form="register" name="contexts">
		<div class="row mx-0 mt-4 px-4 max-w-sm-900">
			<div class="col-12 col-xl-6">
				<div class="row">
					<div class="col-12">
						<h1 class="ammonite-h1-text">{translate key="user.register.contextsPrompt"}</h1>
					</div>
				</div>
								...
								<fieldset form="register" name="roles">
									...
									{if !$enableSiteWidePrivacyStatement && $context->getData('privacyStatement')}
										<div class="form-check my-3 {if !$isSelected}visually-hidden{/if}">
											<input type="checkbox" class="form-check-input" name="privacyConsent[{$contextId}]"
												id="privacyConsent[{$contextId}]" value="1"
												{if $privacyConsent[$contextId]}checked="checked" {/if}
												{* TODO: See if this should use HTML5 required *} />
											<label class="form-check-label ammonite-regular-text mb-0"
												for="privacyConsent[{$contextId}]">
